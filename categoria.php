...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $descricao != "") {
    try {
        if ($id != "") {
            $stmt = $conexao->prepare("UPDATE g1_cargo SET descricao=? WHERE id = ?");
            $stmt->bindParam(1, $descricao);
            $stmt->bindParam(2, $id);
        } else {
            $stmt = $conexao->prepare("INSERT INTO g1_cargo (descricao) VALUES (?)");
        }
        $stmt->bindParam(1, $descricao);
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "upd" && $id != "") {
    try {
        $stmt = $conexao->prepare("SELECT * FROM g1_cargo WHERE id = ?");
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "del" && $id != "") {
    try {
        $stmt = $conexao->prepare("DELETE FROM g1_cargo WHERE id = ?");
        $stmt->bindParam(1, $id, PDO::PARAM_INT);
        if ($stmt->execute()) {
...
<html>
    ...
    <body>
        ...
        <table border="1" width="100%">
                ...
                try {
                    $stmt = $conexao->prepare("SELECT * FROM g1_cargo");
                    if ($stmt->execute()) {
