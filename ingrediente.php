...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "save" && $descricao != "") {
    try {
        if ($id != "") {
            $stmt = $conexao->prepare("UPDATE g1_ingrediente SET nome=?,  descricao=? WHERE id = ?");
            $stmt->bindParam(1, $nome);
            $stmt->bindParam(2, $descricao);
            $stmt->bindParam(3, $id);
        } else {
            $stmt = $conexao->prepare("INSERT INTO g1_ingrediente (nome, descricao) VALUES (?, ?)");
        }
        $stmt->bindParam(1, $nome);
        $stmt->bindParam(2, $descricao);
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "upd" && $id != "") {
    try {
        $stmt = $conexao->prepare("SELECT * FROM g1_ingrediente WHERE id = ?");
...
if (isset($_REQUEST["act"]) && $_REQUEST["act"] == "del" && $id != "") {
    try {
        $stmt = $conexao->prepare("DELETE FROM g1_ingrediente WHERE id = ?");
        $stmt->bindParam(1, $id, PDO::PARAM_INT);
        if ($stmt->execute()) {
...
<html>
    ...
    <body>
        ...
        <table border="1" width="100%">
                ...
                try {
                    $stmt = $conexao->prepare("SELECT * FROM g1_ingrediente");
                    if ($stmt->execute()) {
                        while ($rs = $stmt->fetch(PDO::FETCH_OBJ)) {
                            echo "<tr>";
                            echo "<td>".$rs->nome."</td><td>".$rs->descricao
